...
add_action( 'init', 'ollo_bunch_theme_init');


if( !function_exists( 'ollo_set' ) ) {
	function ollo_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('ollo_printr' ) ) {
	function ollo_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('ollo__font_awesome' ) ) {
	function ollo_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = ollo_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('ollo_load_class' ) ) {
	function ollo_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
ollo_load_class( 'enqueue', 'helpers', false );
ollo_load_class( 'bootstrap_walker', 'helpers', false );


if( ollo_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function ollo_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include OLLO_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include OLLO_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = OLLO_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = OLLO_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( ollo_set( $_GET, 'page' ) == 'ollo'.'_option' ) 
		{
			if( ollo_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'ollo_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = OLLO_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'ollo'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'ollo'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'ollo' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'ollo' ), // menu label
	));
