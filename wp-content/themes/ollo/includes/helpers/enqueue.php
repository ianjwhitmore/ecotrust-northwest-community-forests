 ...
 * @version   1.0
 * @link      http://themeforest.net/user/unifytheme
 * @author    Amir
...
class Bunch_Enqueue
{
	...
	function bunch_enqueue_scripts()
	{
		...
		$header_style = ollo_set( $options, 'header_style' );
		
 
		$version = $current_theme->get( 'Version' );
		
		$dark_color = ( ollo_set( $options, 'website_theme' ) == 'dark' ) ? true : false;
		
		$dark_color = ( ollo_set( $_GET, 'color_style' ) == 'dark' ) ? true : $dark_color;
		
		if(ollo_set($options, 'color_scheme') == 'color2') : $color = 'css/color2.css'; else: $color = 'css/color1.css'; endif;	
		...
		if( is_single() ) {
			...
			if( $format == 'gallery' ) wp_enqueue_script( array( 'ollo_jquery-flexslider' ) );
			if( $format == 'video' || $format == 'audio' ) wp_enqueue_script( array( 'ollo_jquery-fitvids' ) );
		}
		if( is_singular( 'bunch_projects' ) || $wp_query->is_posts_page || is_search() || is_tag() || is_category() || is_author() || is_archive() ) 
  		wp_enqueue_script( array('ollo_jquery-flexslider', 'ollo_owl.carousel', 'ollo_jquery-fitvids') );
		wp_enqueue_script( array('ollo_custom_script') );
		
		wp_add_inline_style( 'ollo-main-style', $this->wp_head() );
		$opt = _WSH()->option();
		$h_js = 'if( ajaxurl === undefined ) var ajaxurl = "'.esc_url( admin_url( 'admin-ajax.php' ) ).'";';
		wp_add_inline_script( 'ollo-main-script', $h_js );
		
		
	}
	
	function ollo_get_font_settings( $styles )
	{
		$opt = _WSH()->option();
		$protocol = ( is_ssl() ) ? 'https' : 'http';
		$font = array();
		...
		if( ollo_set( $opt, 'use_custom_font' ) ){
			...
			if( $h1 = ollo_set( $opt, 'h1_font_family' ) ) $font[$h1] = urlencode( $h1 ).':400,300,600,700,800';
			if( $h2 = ollo_set( $opt, 'h2_font_family' ) ) $font[$h2] = urlencode( $h2 ).':400,300,600,700,800';
			if( $h3 = ollo_set( $opt, 'h3_font_family' ) ) $font[$h3] = urlencode( $h3 ).':400,300,600,700,800';
		}
		
		if( ollo_set( $opt, 'body_custom_font' ) ){
			if( $body = ollo_set( $opt, 'body_font_family' ) ) $font[$body] = urlencode( $body ).':400,300,600,700,800';
		}
